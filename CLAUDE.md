...
- `scripts/translate.mjs` — one-off helper that fills **missing** French project `description`s in `projects.fr.json` via **Microsoft Azure Translator**. Run locally (never in CI): `AZURE_TRANSLATOR_KEY=… AZURE_TRANSLATOR_REGION=canadacentral node scripts/translate.mjs`. Key/region come from env vars only (never committed); it never overwrites existing French, so it's safe to re-run after adding English descriptions. Output (`projects.fr.json`) is committed; the machine draft should get a human review.
