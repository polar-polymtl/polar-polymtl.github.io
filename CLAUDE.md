...
- `src/data/people.json` — the full lab roster. `group` (`director` | `postdoc` | `phd` | `msc` | `cosupervised` | `undergrad` | `alumni-grad` | `intern` | `alumni-undergrad`) decides which section a person renders in on the People page. **Ordering within a section is derived, not stored** — `people.astro` sorts by `years` (end year desc → start year desc → name), so there is no `order` field; a single-year value like `"2026"` counts as `2026–2026`. The `director` and `postdoc` groups are exempt and keep their order in the JSON file. `photo` is optional (falls back to an auto-generated monogram avatar in `PersonCard.astro`). `links` is optional (`scholar`/`linkedin`/`x`/`github`) → social icons.
...
- **People photo carousel:** `Carousel.astro` (scroll-snap track + prev/next). `people.astro` builds its image list at build time by reading **every file in `public/images/team/`** (`fs.readdirSync`, sorted by filename) — so adding a team photo is just dropping a file in that folder; the section hides itself if the folder is empty.
